...
## 8. Hindsight MCP — Persistent Memory (Optional)

Requires PostgreSQL with `pgvector`. When available, replaces `DECISION_LOG.md` as primary memory.

**Resolution protocol:**
```
1. Attempt Hindsight recall → if successful, use it
2. If unavailable → read DECISION_LOG.md
3. If neither exists → ask user: A) Setup Hindsight  B) Create DECISION_LOG.md  C) Start fresh
```

**Operations:** `retain` (after decisions/bug resolutions), `recall` (session start, before decisions), `reflect` (after multi-feature completion).
